...
import 'package:wereward/return_category.dart';
...
class ReturnFormPage extends StatefulWidget {
  const ReturnFormPage({Key key, this.model}) : super(key: key);
  final dynamic model;
  @override
...
class _ReturnFormPageState extends State<ReturnFormPage> {
      ...
      children: [
        ...
        _buildItemBtn(
          ...
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ReturnCategoryPage(model: categoryModel),
            ),
          ).then(
            (value) => setState(() => {
  ...
  StackTap _buildItemBtn(
    ...
    return StackTap(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            categoryModel['title'] != '' ? Container() : icon,
            SizedBox(width: 10),
            categoryModel['title'] != ''
                ? Container()
                : Expanded(
  ...
  _create() async {
    ...
    if (status) {
      return toastFail(context, text: 'กรุณากรอกข้อมูลให้ครบถ้วน');
    } else {
      var data = {
        'code': widget.model[0]['items'][0]['orderNoReference'],
        'isReturn': true,
        'imageUrl': listImage[0]['imageUrl'],
        'description': textDetailController.text,
        'reason': categoryModel['title'],
        'galleries': listImage,
        'status': 'R'
      };

      var response = await postDio('${server}m/cart/order/status/update', data);
